...
public class AllCongesPresenter extends Presenter<AllCongesPresenter.MyView, AllCongesPresenter.MyProxy> {
	...
	@ProxyStandard
	interface MyProxy extends Proxy<AllCongesPresenter> {
	}

	@Inject
	AllCongesPresenter(EventBus eventBus, MyView view, MyProxy proxy) {
	...
	protected void initListBox() {
		getView().getListBox().addItem("All");
		...
		dispatchAsync.execute(new RetrieveGroupAction("All"), new AsyncCallback<RetrieveGroupResult>() {
			...
			public void onSuccess(RetrieveGroupResult result) {
				for (GroupWrapper g : result.getGroup()) {
					getView().getListBox().addItem(g.getName());
	...
	protected void initCalendar() {
		...
		dispatchAsync.execute(new RetrieveGroupAction("All"), new AsyncCallback<RetrieveGroupResult>() {
			...
			public void onSuccess(RetrieveGroupResult result) {
				for (GroupWrapper g : result.getGroup()) {
					if (g.getName().equals(getView().getListBox().getSelectedValue())) {
						for (UserWrapper u : g.getCollaborateurs()) {
							users.add(u);
						}

					} else if (getView().getListBox().getSelectedValue().equals("All")) {
						for (UserWrapper u : g.getCollaborateurs()) {
							users.add(u);
						}
					}
