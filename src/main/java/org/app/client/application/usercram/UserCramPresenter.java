...
public class UserCramPresenter extends Presenter<UserCramPresenter.MyView, UserCramPresenter.MyProxy> {
	...
	@ProxyStandard
	interface MyProxy extends Proxy<UserCramPresenter> {
	}

	@Inject
	UserCramPresenter(EventBus eventBus, MyView view, MyProxy proxy, CurrentUser currentUser) {
							...
							public void onSuccess(UserInformationResult result) {
								...
								if (getView().getClientBox().getSelectedValue().equals("All")) {
									...
									for (WorkLoadWrapper w : result.getResult().getCrams().iterator().next()
											.getWorkList()) {
										if (w.getClientName().contains("-")) {
											String[] list = w.getClientName().split("-");
											if (list[0].equals(getView().getClientBox().getSelectedValue())) {
												theWork.add(new WorkLoadWrapper(list[0], "M", (float) 0.5));
											} else if (list[1].equals(getView().getClientBox().getSelectedValue())) {
												theWork.add(new WorkLoadWrapper(list[1], "A", (float) 0.5));
											} else {
	...
	protected void generatePdf() {
		...
		getView().getSaveCram().addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				dispatchAsync.execute(
						new GeneratePdfAction(userWrapper.getUserEmail(), daysUC,
								getView().getClientBox().getSelectedValue(), currentDate),
						new AsyncCallback<GeneratePdfResult>() {

							@Override
							public void onFailure(Throwable cught) {
								// TODO Auto-generated method stub

							}

							@Override
							public void onSuccess(GeneratePdfResult response) {
								PopupMessageEvent messageEvent;
								messageEvent = new PopupMessageEvent("Le document a été envoyé.");
								eventBus.fireEvent(messageEvent);
								addToPopupSlot(notifyPresenter);
		...
		public void onConfirm(ConfirmEvent event) {
			if (daysUC != null) {
				if (event.getMessage().equals("ok")) {
					if (event.getSender().equals("UserCram")) {
