...
public class NotifyCramHandler implements ActionHandler<NotifyCramAction, NotifyCramResult> {

	@Override
	public NotifyCramResult execute(NotifyCramAction action, ExecutionContext context) throws ActionException {
		User person = null;
		Session session = null;
		Transaction tx = null;
		Date currentDate = new Date(action.getDate().getTime());
		...
		try {
			try {
				...
				tx.commit();
				for (Cram c : person.getCram()) {
					if ((c.getDate().getYear() == currentDate.getYear())
							&& (c.getDate().getMonth() == currentDate.getMonth())) {
						...
						file = person.getName() + "-" + person.getSurname() + "-" + month(currentDate.getMonth() + 1)
								+ "-" + (currentDate.getYear() + 1900) + ".pdf";
						try {
							...
							Paragraph mois = new Paragraph(
									month(c.getDate().getMonth() + 1) + "-" + (c.getDate().getYear() + 1900),
									smallBold);
							...
							preface.add(new Paragraph("Récapitulatif:", smallBold));
							if (congePaye > 0)
							...
							c1 = new PdfPCell(new Phrase("Client", subFont));
							c1.setHorizontalAlignment(Element.ALIGN_CENTER);
							table.addCell(c1);
							...
							String dayLoad = "";
							PdfPCell cell;
							Date workingDate = new Date(currentDate.getTime());
							...
							for (WorkLoadWrapper w : action.getWork()) {
								...
								if (c.getDate().getMonth() + 1 < 10)
									month = "0" + (c.getDate().getMonth() + 1);
								else
									month = "" + (c.getDate().getMonth() + 1);

								workingDate.setDate(compteur);
								cell = new PdfPCell(
										new Phrase(day + "/" + month + "/" + (c.getDate().getYear() + 1900)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								if (workingDate.getDay() == 0 || workingDate.getDay() == 6)
									cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								cell = new PdfPCell(new Phrase(w.getClientName()));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								if (workingDate.getDay() == 0 || workingDate.getDay() == 6)
									cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
						...
						//csvFile = person.getName() + "-" + person.getSurname() + "-" + month(currentDate.getMonth() + 1)
						//		+ "-" + (currentDate.getYear() + 1900) + ".csv";
...
//									writer.println(day + "/" + month + "/" + (c.getDate().getYear() + 1900) + ";"
...
//						}
				...
				SendMail.execute(action.getEmail(), "CRAM " + month(currentDate.getMonth() + 1),
						"Bonjour, \nVeuilez trouver ci-joint le cram du mois : " + month(currentDate.getMonth() + 1),
						file);

				SendMail.execute(email,
						"CRAM " + month(currentDate.getMonth() + 1) + " " + person.getName() + " "
								+ person.getSurname(),
						"Bonjour, \nVeuilez trouver ci-joint le cram du mois : " + month(currentDate.getMonth() + 1),
						file);
		...
		logger.info(person.getName() + " cram of the month " + month(currentDate.getMonth() + 1)
				+ " is successfully generated");
		return new NotifyCramResult("ok");
	}

	@Override
	public Class<NotifyCramAction> getActionType() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public void undo(NotifyCramAction action, NotifyCramResult result, ExecutionContext context)
